...
def preprocess(image, mask, tmask,flip=False, scale=None, crop=None):
  if flip:
    if random.random() < 0.5:
      image = image.transpose(Image.FLIP_LEFT_RIGHT)
      mask = mask.transpose(Image.FLIP_LEFT_RIGHT)
      tmask = tmask.transpose(Image.FLIP_LEFT_RIGHT)	
  if scale:
    w, h = image.size
    ...
    tmask = tmask.resize(new_size, Image.NEAREST)	
  ...
  tmask = torch.LongTensor(np.array(tmask).astype(np.int64))
  ...
  if crop:
    ...
    tmask = torch.nn.ConstantPad2d((0, pad_lr, 0, pad_tb), 255)(tmask)
    ...
    tmask = tmask[i:i + crop[0], j:j + crop[1]]
