...
class Cityscapes(data.Dataset):
  ...
  def __init__(self, root, train=True, transform=None, target_transform=None, download=False, crop_size=None):
    ...
    self.tmasks = self._get_files('label', 'trained_labelID')

  def __getitem__(self, index):
    _img = Image.open(self.images[index]).convert('RGB')
    _target1 = Image.open(self.masks[index])
    _target2 = Image.open(self.tmasks[index])

    _img, _target1, _target2 = preprocess(_img, _target1, _target2,
    ...
    if self.target_transform is not None:
      _target1 = self.target_transform(_target1)
      _target2 = self.target_transform(_target2)

    return _img, _target1, _target2
